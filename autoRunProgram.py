...
class AutoRunProgramGUI:
    ...
    def create_widgets(self):
        ...
        # 小时选择
        hour_spin = ttk.Spinbox(self.shutdown_countdown_frame, from_=0, to=23, width=5, textvariable=self.shutdown_hour_var)
        hour_spin.pack(side="left", padx=2)
        ttk.Label(self.shutdown_countdown_frame, text="时").pack(side="left")
        
        # 分钟选择
        minute_spin = ttk.Spinbox(self.shutdown_countdown_frame, from_=0, to=59, width=5, textvariable=self.shutdown_minute_var)
        minute_spin.pack(side="left", padx=2)
        ttk.Label(self.shutdown_countdown_frame, text="分").pack(side="left")
        
        # 秒钟选择
        second_spin = ttk.Spinbox(self.shutdown_countdown_frame, from_=0, to=59, width=5, textvariable=self.shutdown_second_var)
        second_spin.pack(side="left", padx=2)
        ...
        # 重启设置框架
        restart_frame = ttk.LabelFrame(self.root, text="重启设置", padding="10")
        restart_frame.pack(fill="x", padx=10, pady=5)
        
        # 重启模式选择
        restart_mode_frame = ttk.Frame(restart_frame)
        restart_mode_frame.pack(fill="x", pady=5)
        self.restart_mode = tk.StringVar(value="countdown")
        ttk.Radiobutton(restart_mode_frame, text="倒计时重启", variable=self.restart_mode, 
                       value="countdown", command=self.update_restart_widgets).pack(side="left", padx=5)
        ttk.Radiobutton(restart_mode_frame, text="定时重启", variable=self.restart_mode, 
                       value="scheduled", command=self.update_restart_widgets).pack(side="left", padx=5)
        
        # 重启时间设置框架
        self.restart_time_frame = ttk.Frame(restart_frame)
        self.restart_time_frame.pack(fill="x", pady=5)
        
        # 倒计时重启设置
        self.restart_countdown_frame = ttk.Frame(self.restart_time_frame)
        self.restart_countdown_frame.pack(fill="x")
        
        ttk.Label(self.restart_countdown_frame, text="倒计时时间:").pack(side="left", padx=5)
        self.restart_hour_var = tk.StringVar(value="00")
        self.restart_minute_var = tk.StringVar(value="00")
        self.restart_second_var = tk.StringVar(value="00")
        
        # 小时选择
        hour_spin = ttk.Spinbox(self.restart_countdown_frame, from_=0, to=23, width=5, textvariable=self.restart_hour_var)
        hour_spin.pack(side="left", padx=2)
        ttk.Label(self.restart_countdown_frame, text="时").pack(side="left")
        
        # 分钟选择
        minute_spin = ttk.Spinbox(self.restart_countdown_frame, from_=0, to=59, width=5, textvariable=self.restart_minute_var)
        minute_spin.pack(side="left", padx=2)
        ttk.Label(self.restart_countdown_frame, text="分").pack(side="left")
        
        # 秒钟选择
        second_spin = ttk.Spinbox(self.restart_countdown_frame, from_=0, to=59, width=5, textvariable=self.restart_second_var)
        second_spin.pack(side="left", padx=2)
        ttk.Label(self.restart_countdown_frame, text="秒").pack(side="left")
        
        # 定时重启设置
        self.restart_scheduled_frame = ttk.Frame(self.restart_time_frame)
        
        ttk.Label(self.restart_scheduled_frame, text="重启时间:").pack(side="left", padx=5)
        self.restart_date_var = tk.StringVar(value=datetime.datetime.now().strftime("%Y-%m-%d"))
        self.restart_time_var = tk.StringVar(value=datetime.datetime.now().strftime("%H:%M:%S"))
        
        # 日期选择
        date_entry = ttk.Entry(self.restart_scheduled_frame, textvariable=self.restart_date_var, width=10)
        date_entry.pack(side="left", padx=2)
        ttk.Label(self.restart_scheduled_frame, text="时间:").pack(side="left")
        
        # 时间选择
        time_entry = ttk.Entry(self.restart_scheduled_frame, textvariable=self.restart_time_var, width=8)
        time_entry.pack(side="left", padx=2)
        
        # 重启控制按钮
        restart_control_frame = ttk.Frame(restart_frame)
        restart_control_frame.pack(fill="x", pady=5)
        
        # 设置重启按钮
        set_restart_btn = ttk.Button(restart_control_frame, text="设置重启", command=self.set_restart)
        set_restart_btn.pack(side="left", padx=5)
        
        # 取消重启按钮
        cancel_restart_btn = ttk.Button(restart_control_frame, text="取消重启", command=self.cancel_restart)
        cancel_restart_btn.pack(side="left", padx=5)
    ...
    def update_restart_widgets(self):
        if self.restart_mode.get() == "countdown":
            self.restart_scheduled_frame.pack_forget()
            self.restart_countdown_frame.pack(fill="x")
        else:
            self.restart_countdown_frame.pack_forget()
            self.restart_scheduled_frame.pack(fill="x")
            # 设置默认时间为当前时间后1分钟
            default_time = datetime.datetime.now() + datetime.timedelta(minutes=1)
            self.restart_date_var.set(default_time.strftime("%Y-%m-%d"))
            self.restart_time_var.set(default_time.strftime("%H:%M:%S"))
    ...
    def set_restart(self):
        try:
            if self.restart_mode.get() == "countdown":
                hours = int(self.restart_hour_var.get())
                minutes = int(self.restart_minute_var.get())
                seconds = int(self.restart_second_var.get())
                if hours == 0 and minutes == 0 and seconds == 0:
                    messagebox.showerror("错误", "请设置倒计时时间！")
                    return
                time_settings = {
                ...
                try:
                    scheduled_time = datetime.datetime.strptime(
                        f"{self.restart_date_var.get()} {self.restart_time_var.get()}",
                        "%Y-%m-%d %H:%M:%S"
                    )
                    if scheduled_time < datetime.datetime.now():
                        messagebox.showerror("错误", "指定的时间已经过去！")
                    ...
                    messagebox.showerror("错误", "请输入正确的时间格式！(YYYY-MM-DD HH:MM:SS)")
                    return
            self.restart_task = RestartTask(
                self.restart_mode.get(),
                time_settings
            )
            self.start_restart()
        except ValueError:
    ...
    def cancel_restart(self):
        if self.restart_task and self.restart_task.is_counting:
            self.restart_task.is_counting = False
            self.log("已取消重启")
            os.system("shutdown /a")

    def start_restart(self):
        if not hasattr(self, 'restart_task') or not self.restart_task:
            return
        try:
            if self.restart_task.execution_mode == "countdown":
                total_seconds = (self.restart_task.time_settings['hours'] * 3600 +
                               self.restart_task.time_settings['minutes'] * 60 +
                               self.restart_task.time_settings['seconds'])
            else:
                wait_seconds = (self.restart_task.time_settings['scheduled_time'] -
                              datetime.datetime.now()).total_seconds()
                total_seconds = int(wait_seconds)
            self.restart_task.is_counting = True
            self.restart_task.countdown_thread = threading.Thread(
                target=self.restart_countdown,
                args=(total_seconds,)
            )
            self.restart_task.countdown_thread.daemon = True
            self.restart_task.countdown_thread.start()
            self.log("开始重启倒计时")
        except Exception as e:
            self.log(f"设置重启时出错: {str(e)}")
            self.restart_task.is_counting = False

    def restart_countdown(self, total_seconds):
        while total_seconds > 0 and self.restart_task.is_counting:
            hours = total_seconds // 3600
            minutes = (total_seconds % 3600) // 60
            seconds = total_seconds % 60
            self.log(f"重启倒计时: {hours:02d}:{minutes:02d}:{seconds:02d}")
            time.sleep(1)
            total_seconds -= 1
        if self.restart_task.is_counting:
    ...
    def execute_restart(self):
        try:
            ...
            self.log(f"重启时出错: {str(e)}")
            messagebox.showerror("错误", f"重启时出错: {str(e)}")
        finally:
            self.restart_task.is_counting = False
